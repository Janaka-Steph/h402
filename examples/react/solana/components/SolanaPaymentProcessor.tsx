...
>(({ isPromptValid, isProcessing, setIsProcessing, prompt = "" }, ref) => {
  ...
  // Since the component is only rendered when selectedAccount is available (conditional rendering in parent),
  // we can safely use the hook here without violating React's Rules of Hooks
  // We need to add a type assertion because TypeScript doesn't know about the conditional rendering in the parent
  ...
  */
  ...
  const handlePayment = useCallback(async (): Promise<void> => {
    // First check if the prompt is valid and we're not already processing
    if (!isPromptValid() || isProcessing) {
      console.log("Payment not valid or already processing");
      return Promise.resolve();
    }
    
    // Then check if we have a wallet account connected
    if (!selectedAccount) {
      console.error("No wallet account connected");
      setStatusMessage("Error: Please connect your Solana wallet first");
      setPaymentStatus("failed");
      return Promise.resolve();
    }
    
    // Check if transaction signer is available
    if (!transactionSendingSigner.signAndSendTransactions) {
      console.error("Transaction signer not available");
      setStatusMessage("Error: Wallet not properly connected. Please reconnect your wallet.");
      setPaymentStatus("failed");
      return Promise.resolve();
    }
    ...
    try {
      ...
      const paymentClient: PaymentClient = {
        solanaClient: {
          ...
          rpc: proxiedRpc,
          // We've already checked that signAndSendTransactions is available
