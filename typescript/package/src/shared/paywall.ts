...
export function getPaywallHtml({
  ...
  const selectedPaymentRequirements = paymentRequirements.find(pr => 
    pr.networkId === (testnet ? "base-sepolia" : "base") && pr.scheme === "exact"
...
<script type="module">
  ...
  window.x402.solana = {
    ...
    createPayment: async (paymentDetails) => {
      ...
      const paymentHeader = await createPayment(paymentDetails, paymentClient);
  ...
  payButton.addEventListener('click', async () => {
    ...
    try {
      ...
      if (window.x402.utils.isSolanaNetwork(selectedNetwork)) {
        ...
        const paymentHeader = await window.x402.solana.createPayment(solanaPaymentDetails);
        
        // Redirect with payment header
        window.location.href = window.x402.currentUrl + '?402base64=' + encodeURIComponent(paymentHeader);
        return;
      }
